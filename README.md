
<div align="center">

# ☕ coff33ninja

**Windows automation • AI experiments • VRM companions • OSINT tools**

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=6EA8FF&center=true&vCenter=true&width=520&lines=Windows+automation+%7C+AI+experimentation+%7C+VRM+development+%7C+OSINT+tools;Automation+scripts+%7C+AI/ML+projects+%7C+Cross-platform+development;South+Africa+%F0%9F%87%BF%F0%9F%87%A6)](https://github.com/coff33ninja)

</div>

> Windows automation tinkerer from Saldanha 🇿🇦. I build tools because I'd rather build them than pay for them — batch scripts → Python GUIs → MCP servers → a from-scratch OS. Boredom is a hell of a motivator.

## 🎯 Now

- 🤖 **JARVIS_SYSTEM + My_Cam** — gesture + voice computer control; My_Cam turns a phone into a webcam, go-mcp-computer-use adds desktop hands
- ⚙️ **go-mcp-computer-use** — teaching a computer to use itself (adaptive MCP server)
- 🛠️ **DJ-TECH** — repair shop management: job cards, inventory, invoicing, WhatsApp
- 🧠 **ai-memory** — giving AI assistants a memory that survives the session
- 🔬 **NO_OS** — a TempleOS-inspired OS from scratch, because why not

## 📝 Up Next

- [ ] **Demo clip** — screen recording of go-mcp-computer-use using itself (screenshot → OCR → click)
- [ ] **Toolkit GIFs** — 30-second GIF in each AI toolkit's README
- [ ] **NO_OS devlog** — weekly progress screenshots
- [ ] **Release push** — tag a go-mcp-computer-use release, post it to the MCP directory + r/LLMDevs
- [ ] **Feedback loop** — [suggest a build](https://github.com/coff33ninja/coff33ninja/issues/new?template=build-request.md) — the template is live, the replies are up to you
- [ ] **Outward contribution** — one PR per week to a tool I depend on
- [ ] **Video uploads** — web videos of the toolkits and AI interacting (mostly free-API AI demos)

## 🧭 Coming Next

- **VRM companion** — a virtual avatar that actually shows up alongside the AI
- **Mobile app** — one of these tools, on Android
- **Next-gen System Optimizer** — the PowerShell era, ending in a real GUI

## 🧭 Last 10 Repos

> Projects built as ideas show up, to solve annoyances, or because I hate paying for something I can build (often with help).

<sub>Auto-sorted by last push · Forks/archived excluded</sub>

<!--START_SECTION:recent_projects-->
<div align="center">

[![go-mcp-computer-use](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-go-mcp-computer-use.svg)](https://github.com/coff33ninja/go-mcp-computer-use)
[![invoice-ninja-portable](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-invoice-ninja-portable.svg)](https://github.com/coff33ninja/invoice-ninja-portable)
[![ai-memory](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-ai-memory.svg)](https://github.com/coff33ninja/ai-memory)
[![blender-mcp](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-blender-mcp.svg)](https://github.com/coff33ninja/blender-mcp)
[![agent-switcher](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-agent-switcher.svg)](https://github.com/coff33ninja/agent-switcher)
[![ai-skills](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-ai-skills.svg)](https://github.com/coff33ninja/ai-skills)
[![AI_ToolSet](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-AI_ToolSet.svg)](https://github.com/coff33ninja/AI_ToolSet)
[![DJ-TECH](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-DJ-TECH.svg)](https://github.com/coff33ninja/DJ-TECH)
[![JARVIS_SYSTEM](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-JARVIS_SYSTEM.svg)](https://github.com/coff33ninja/JARVIS_SYSTEM)
[![GIT_REPO_EXPLORER](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-GIT_REPO_EXPLORER.svg)](https://github.com/coff33ninja/GIT_REPO_EXPLORER)

</div>
<!--END_SECTION:recent_projects-->

## 🧰 AI Toolkits

> The toolchain behind my AI-assisted (Vibecode) workload.

<div align="center">

[![ai-skills](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-ai-skills.svg)](https://github.com/coff33ninja/ai-skills)
[![ai-memory](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-ai-memory.svg)](https://github.com/coff33ninja/ai-memory)
[![go-mcp-computer-use](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-go-mcp-computer-use.svg)](https://github.com/coff33ninja/go-mcp-computer-use)
[![mcp-system-cleanup](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-mcp-system-cleanup.svg)](https://github.com/coff33ninja/mcp-system-cleanup)

</div>

## 🤖 JARVIS System

> Gesture + voice computer control — webcam tracking, a multi-monitor HUD, and an LLM agent. Local-first.

<div align="center">

[![JARVIS_SYSTEM](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-JARVIS_SYSTEM.svg)](https://github.com/coff33ninja/JARVIS_SYSTEM)
[![My_Cam](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-My_Cam.svg)](https://github.com/coff33ninja/My_Cam)

</div>

**How it fits together** — go-mcp-computer-use works alongside My_Cam to give the system hands on the desktop (mouse, keyboard, OCR, window control). It can see, hear, and act.

## 🖥️ System Optimizer

> The evolution of my Windows optimization work — from batch scripts to a real PowerShell tool.

<div align="center">

<a href="https://github.com/coff33ninja/AIO"><img src="https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-AIO.svg" width="230" alt="AIO"></a> → <a href="https://github.com/coff33ninja/NexTool-Windows-Suite"><img src="https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-NexTool-Windows-Suite.svg" width="230" alt="NexTool"></a> → <a href="https://github.com/coff33ninja/System_optimizer"><img src="https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/pin-System_optimizer.svg" width="230" alt="System Optimizer"></a>

<sub><b>1st Gen</b> · Batch</sub> &nbsp;→&nbsp; <sub><b>2nd Gen</b> · Python</sub> &nbsp;→&nbsp; <sub><b>Current</b> · PowerShell</sub>

**Quick start**
```powershell
irm "https://raw.githubusercontent.com/coff33ninja/System_Optimizer/main/run_optimization.bat" -OutFile "run_optimization.bat"
.\run_optimization.bat
```

</div>

**Why it matters** — One script, no dependencies, built for Windows 10/11. 29 optimization options with sensible defaults. The culmination of years of Windows automation work.

## 📈 GitHub Stats

<div align="center">

[![GitHub stats](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/stats.svg)](https://github.com/coff33ninja)
[![Top Languages](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/top-langs.svg)](https://github.com/coff33ninja)

![Profile Views](https://komarev.com/ghpvc/?username=coff33ninja&style=flat-square&color=6EA8FF)
[![GitHub Streak](https://streak-stats.demolab.com/?user=coff33ninja&theme=github-dark-blue&hide_border=true)](https://github.com/coff33ninja)

**Tech stack**

![Go](https://img.shields.io/badge/Go-00ADD8?style=for-the-badge&logo=go&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=for-the-badge&logo=powershell&logoColor=white)
![Batch](https://img.shields.io/badge/Batch-4EAA25?style=for-the-badge&logo=windowsterminal&logoColor=white)

<br>

[![Trophies](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/trophy.svg)](https://github.com/coff33ninja/github-profile-trophy)

</div>

## 📊 Recent Activity

<!--START_SECTION:activity-->
1. 🚀 Pushed to [coff33ninja/invoice-ninja-portable](https://github.com/coff33ninja/invoice-ninja-portable)
2. 🚀 Pushed to [coff33ninja/blender-mcp](https://github.com/coff33ninja/blender-mcp)
3. 🌱 Created branch [main](https://github.com/coff33ninja/agent-switcher/tree/main) in [coff33ninja/agent-switcher](https://github.com/coff33ninja/agent-switcher)
4. 🚀 Pushed to [coff33ninja/go-mcp-computer-use](https://github.com/coff33ninja/go-mcp-computer-use)
5. 🚀 Pushed to [coff33ninja/blender-mcp](https://github.com/coff33ninja/blender-mcp)
6. 🚀 Pushed to [coff33ninja/go-mcp-computer-use](https://github.com/coff33ninja/go-mcp-computer-use)
7. 🚀 Pushed to [coff33ninja/blender-mcp](https://github.com/coff33ninja/blender-mcp)
8. 🚀 Pushed to [coff33ninja/go-mcp-computer-use](https://github.com/coff33ninja/go-mcp-computer-use)
9. 🚀 Pushed to [coff33ninja/blender-mcp](https://github.com/coff33ninja/blender-mcp)
10. 🚀 Pushed to [coff33ninja/go-mcp-computer-use](https://github.com/coff33ninja/go-mcp-computer-use)

<!--END_SECTION:activity-->

## 🌍 About

- 🇿🇦 **Location:** Saldanha, Western Cape, South Africa
- 💼 **Journey:** HLT Computers → Batch scripts → Python GUI → AI & Automation → Mobile Apps
- 🔋 **DIY:** Home automation with solar, battery, and LED matrices
- 📺 **Interests:** Manga/anime, AI experimentation, building stuff I don't want to pay for as a service

## 🎓 Currently Learning

- **VRM / 3D** — virtual companions that show up as more than a chat box
- **Mobile dev** — Android, already shipping in [My_Cam](https://github.com/coff33ninja/My_Cam)
- **OS internals** — because [NO_OS](https://github.com/coff33ninja/NO_OS) isn't going to write itself

## 📫 Connect

<div align="center">

[![YouTube](https://img.shields.io/badge/YouTube-coff33ninja-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@coff33ninja)
[![GitHub](https://img.shields.io/badge/GitHub-coff33ninja-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/coff33ninja)
[![Site](https://img.shields.io/badge/Site-coff33ninja.github.io-181717?style=for-the-badge&logo=githubpages&logoColor=white)](https://coff33ninja.github.io/coff33ninja/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-DJ%20Kruger-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dj-kruger-5825aa228/)

</div>

<div align="center">

### 🐍 Contribution Graph

![Snake animation](https://raw.githubusercontent.com/coff33ninja/coff33ninja/output/github-contribution-grid-snake-dark.svg)

⭐ **If you find my projects helpful, consider giving them a star!**

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/V1R824VMV5)

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer"/>

</div>





<!-- debug: force activity update -->
